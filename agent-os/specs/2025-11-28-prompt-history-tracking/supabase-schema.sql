...
COMMENT ON TABLE conversations IS 'Stores complete AI chat conversation sessions with metadata and tracking';
COMMENT ON TABLE conversation_messages IS 'Stores individual messages within conversations with role and content';
COMMENT ON TABLE conversation_tags IS 'Stores user-defined tags for conversation categorization';
COMMENT ON COLUMN conversations.metadata IS 'JSON metadata for additional conversation attributes like context blocks, settings, etc.';
COMMENT ON COLUMN conversation_messages.metadata IS 'JSON metadata for additional message attributes like token details, timing, etc.';
COMMENT ON COLUMN conversations.original_prompt_content IS 'The complete assembled prompt that initiated the conversation';
COMMENT ON COLUMN conversations.context_block_ids IS 'Array of context block IDs used in the original prompt';
COMMENT ON COLUMN conversations.token_usage IS 'Total tokens used across all messages in the conversation';
COMMENT ON COLUMN conversations.execution_duration_ms IS 'Total execution time in milliseconds for the conversation';
COMMENT ON COLUMN conversations.estimated_cost IS 'Estimated cost in USD for the conversation based on token usage';